...
class YBN(Plugin):
    ...
    def reset(self : YBN, project_path : Path = Path(), filename : str = "") -> None:
        ...
        if p != self.last_ypf_path:
            if not self.load_last_ypf_from_metadata(project_path.parts[1], Path(filename).parent.parent):
                # old, fallback method
                self.load_last_ypf(p)

    def load_last_ypf_from_metadata(self : YBN, name : str, target_path : Path) -> bool:
        try:
            target_data : str = target_path.as_posix() + " YPF Data"
            if target_data in self.owner.projects[name]["metadata"]:
                self.last_ypf_data = json.loads(self.owner.projects[name]["metadata"][target_data])
    ...
    # fallback method, loading data from a local ypf.json file
